...
export const DEMO_PHP_CODE = `<?php
/*
  $Id$
...
export const ADMIN_PORTAL_MAIN = `<?php
session_start();
extract($_REQUEST); 
require_once('includes/auth_logic.php');

$db_conn = mysql_connect("localhost", "root", "root123");
mysql_select_db("legacy_shop", $db_conn);

$msg = "";

if ($action == 'login') {
    $user_data = validate_user($username, $password, $db_conn);
    if ($user_data) {
        ...
        $msg = "Welcome back, " . $user_data['username'];
    } else {
        $msg = "Invalid Login!";
    }
}
...
if ($_SESSION['admin_user']) {
    ...
    if ($_COOKIE['debug_mode'] == 'dev_bypass_99') {
        echo "<pre>"; print_r($GLOBALS); echo "</pre>";
    }
} else {
    echo '<form method="post" action="admin_gate.php">';
...
export const ADMIN_PORTAL_DEP = `<?php
function validate_user($u, $p, $conn) {
    $hashed_pass = md5($p);
    $sql = "SELECT * FROM users WHERE username = '$u' AND password = '$hashed_pass'";
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        mysql_query("UPDATE users SET last_login = NOW() WHERE id = " . $row['id']);
        return $row;
    }
    return false;
}
